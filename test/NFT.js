...
describe("NFT", async function () {
  it("should be able to mint, transferFrom, burn. And it should return appropriate name, symbol, totalSupply, tokenURI, ownerOf, balanceOf", async function () {
    ...
    expect(await nft.name()).to.equal("ERC721 Deploy Test");
    expect(await nft.symbol()).to.equal("DT");
    ...
    expect(await nft.tokenURI(0)).to.equal("https://example.com/tokens_uri/0")
    ...
    expect(await nft.tokenURI(1)).to.equal("https://example.com/tokens_uri/1")
    ...
    expect(await nft.tokenURI(2)).to.equal("https://example.com/tokens_uri/2");
